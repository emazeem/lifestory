        ...
        <div class="main-content">
            <div class="page-header">
                <div class="container-fluid d-sm-flex justify-content-between">
                    <h4 class="font-size-25">Customers</h4>
                                    ...
                                    <tr v-if="users.data && users.data.length > 0" v-for="(user,index) in users.data" :key="index">
                                        <td>
                                            ...
                                            <router-link :to="'user/view/'+user.id" class="text-primary d-flex align-items-center" >
                                                <img style="width: 30px;height: 30px;object-fit: cover;border-radius: 50%" :src="user.profile" class="d-md-block d-none " />
                                        ...
                                        <td class="text-center">
                                            <button @click="deleteCustomer(user.id)" class="btn btn-danger cursor-pointer" title="Delete customer">
...
export default {
    ...
    methods: {
            ...
            swal({
                    ...
                    if (result) {
                        this.axios.post('user/delete', { 'id': id }).then(() => {
                            this.$moshaToast({ title: 'Customer deleted successfully' }, { type: 'success', showIcon: true, hideProgressBar:true });
                            this.list(1);
                        }).catch(()=>{
        ...
        async list(page = 1) {
            await this.axios.get(`user/fetch-all?role=customer&page=${page}&search=${this.search}`)
                .then(({ data }) => {
